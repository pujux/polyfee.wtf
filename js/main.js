...
async function init() {
  ...
  do {
    ...
    let response = await fetch(
      'https://corsproxy.io/?' + encodeURIComponent(`https://api.polygonscan.com/api?module=account&action=txlist&address=${address}&startblock=${fromBlock}&endblock=99999999&sort=asc&apikey=${key}`)
    );
    if (response.ok) {
